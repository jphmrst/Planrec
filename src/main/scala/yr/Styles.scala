...
extends AutomatonStyle[HState[T, H, S], H](
  id = id,
  ...
  nodeShape = (s: HState[T, H, S], _: Graphable[HState[T, H, S], H, ?])
    => s match {
      ...
      case _: Sparking[_, _, _]  => "rectangle"
      case _ => "circle"
    },
...
extends AutomatonStyle[Set[HState[T, H, S]], H](
  ...
  nodeShape = (
    s: Set[HState[T, H, S]], _: Graphable[Set[HState[T, H, S]], H, ?]
  ) => "rectangle",
...
def nodeDOT[T, H, S](node: HState[T, H, S]): String = node match {
  ...
  case Sparking(indirects, item) =>
    "<sup>{" + indirects.map(_.toString()).mkString(",") + "}</sup>"
    + nodeDOT(item)
